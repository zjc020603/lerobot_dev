...
class SiglipGemmaValueModel(PreTrainedModel):
    ...
    def forward(
        ...
        attention_mask = rearrange(attention_mask, "b n1 n2 -> b 1 n1 n2")
        mask_mapping = {
        ...
        if final_hidden.dtype != self.value_head.weight.dtype:
            final_hidden = final_hidden.to(dtype=self.value_head.weight.dtype)
